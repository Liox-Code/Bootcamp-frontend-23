(() => {
 ...
 console.log(smallestNumber)
  ...
  const countELementsArray = ex02Array.reduce((countedElements, item) => {
    countedElements[item] = (countedElements[item] || 0) + 1
    return countedElements
  }, {})
  const minCount = Math.min(...Object.values(countELementsArray))
  const lessFrequent = Object.keys(countELementsArray).filter((key) => countELementsArray[key] === minCount)
  ...
  // ****************************** EXERCISE 3 ******************************
   ...
   const uniqueArrayOFElements = ex03Array.filter((item) => {
     if(elementsObject[item]){
       return false
     } else{
       elementsObject[item] = true;
       return true
     }
   })
   ...
   console.log(uniqueArrayOFElements)
